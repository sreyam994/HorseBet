...
// Uses data-testid attributes (stable)

console.log('[Ladbrokes Scraper] Initializing on:', window.location.href);
  ...
  runnerRows.forEach(row => {
    ...
    if (nameElement) {
      ...
      if (name.toLowerCase() === horseName.toLowerCase()) {
        ...
        console.log('[Ladbrokes Scraper] Highlighted:', horseName);
      }
    }
  });
}
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.action === 'request_odds') {
    ...
    scrapeOdds(targetNames)
      ...
      .catch(error => {
        console.error('[Ladbrokes Scraper] Error:', error);
        sendResponse({
          success: false,
          error: error.message,
          data: []
...
function scrapeOdds(targetHorseNames = []) {
  return new Promise((resolve) => {
    ...
    try {
      ...
      console.log(`[Ladbrokes Scraper] Found ${runnerRows.length} runner rows`);
      ...
      runnerRows.forEach((row, index) => {
        try {
          ...
          const nameElement = row.querySelector('span[data-testid="runner-name"]');
          if (!nameElement) {
            console.warn(`[Ladbrokes Scraper] Row ${index}: No name element found`);
            return;
          }
          ...
          if (targetHorseNames.length > 0) {
            ...
            if (matchedName) {
              ...
              console.log(`[Ladbrokes Scraper] Matched "${horseName}" → Betfair: "${matchedName}"`);
            } else {
              console.log(`[Ladbrokes Scraper] No match for "${horseName}", skipping`);
              return;
            }
          }
          ...
          if (!oddsColumn) {
            console.warn(`[Ladbrokes Scraper] Row ${index} (${horseName}): No odds column found`);
            return;
          }
          ...
          if (!priceButton) {
            console.warn(`[Ladbrokes Scraper] Row ${index} (${horseName}): No price button found`);
            return;
          }
          ...
          if (!winOddsElement) {
            console.warn(`[Ladbrokes Scraper] Row ${index} (${horseName}): No win odds element found`);
            return;
          }
          ...
          if (isNaN(winOdds)) {
            console.warn(`[Ladbrokes Scraper] Row ${index} (${horseName}): Invalid win odds "${winOddsText}"`);
            return;
          }
          ...
          console.log(`[Ladbrokes Scraper] Extracted: ${finalName} @ Back ${winOdds}`);
        } catch (error) {
          console.error(`[Ladbrokes Scraper] Error processing row ${index}:`, error);
        }
      });

      console.log(`[Ladbrokes Scraper] Successfully extracted ${horses.length} horses`);
      resolve(horses);
    } catch (error) {
      console.error('[Ladbrokes Scraper] Fatal error:', error);
      resolve([]);
    }
  });
}
